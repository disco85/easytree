...
(defstruct <tree-state>
  ...
  ;; the last (previous) file/dir needed to detect :find when it could be treated as :tree
  ;; ("pure" bcs it is as it was read, prefix was not added - to be abs./full path):
  (last-pure-fname nil :type (or null string)))
...
(declaim (ftype (function (<parsed-lines> <format>) integer) get-parsed-lines-hits))
(defun get-parsed-lines-hits (PARSED-LINES FORMAT)
  (ecase FORMAT
    (:tree (<parsed-lines>-tree-hits PARSED-LINES))
    (:find (<parsed-lines>-find-hits PARSED-LINES))
    (:ls1r (<parsed-lines>-ls1r-hits PARSED-LINES))
    (:ls1rl (<parsed-lines>-ls1rl-hits PARSED-LINES))))
                                ...
                                (let ((k (car x)))
                                  (cond ((null acc)
                                        ...
                                        ((= k (car (first acc)))
                                         (cons x acc))
                                        (t acc))))
                              fmts-with-parsed-lines
                              :initial-value nil
                              :from-end nil))
         (res (mapcar #'cdr accumulated)))
    res))
...
(defun split-path (path &key (strip-end nil) (parts-num nil parts-num-supplied))
...
(defun subpath-p (parent child &key (equal-too t))
  "Return true if CHILD is inside PARENT (POSIX-style, purely textual)."
  (let* ((p (split-path parent :strip-end t))
         (c (split-path child :strip-end t))
         (cmp-fun (if equal-too #'<= #'<)))
    (when (and parent child)
      (and (funcall cmp-fun (length p) (length c))
           (equal p (subseq c 0 (length p)))))))
...
;; Also they return different things on success. But NIL of failure.
(declaim (ftype (function (<parsed-lines> string) (or null t)) parse-line-as-tree))
(declaim (ftype (function (<parsed-lines> string) (or null t)) parse-line-as-find))
    ...
    (when marked-up-str
      (symbol-macrolet ;; XXX "let" working as #define (macro-substitution), so save setf-ability
          ((state-indents (<tree-state>-indents (<parsed-lines>-tree-state PARSED-LINES)))
           ...
           (last-state-pure-fname (<tree-state>-last-pure-fname (<parsed-lines>-tree-state PARSED-LINES))))
        (setf
         parsed-line
         (progn
           ...
           (if (and (<parsed-lines>-dir-from-final-slash PARSED-LINES)
               ...
               (setf fsobj :dir)
               (setf fsobj :file))
           (dbg (format nil "!!!!!! last-state-pure-fname: ~A  fname: ~A  fname-start: ~A  last-state-indent: ~A  SUBPATH-P: ~A~%"
                        last-state-pure-fname fname fname-start last-state-indent
                        (subpath-p last-state-pure-fname fname :equal-too nil)))
           (cond
             ...
             ((> fname-start last-state-indent)
              ...
              (make-<parsed-line> :format :tree :fname fname :fsobj fsobj))
             ((and (= fname-start last-state-indent)
                   ;; To prevent mismatching of :FIND format we check that we have not
                   ;; fname-s that start from the same indentation as the previous line
                   ;; but being prefixed with the prev. fname (pure-fname - without
                   ;; prepending fname with the contextual dir), ie, we reject fnames:
                   ;;   ./a/b     <-- last-state-pure-fname
                   ;;   ./a/b/c   <-- reject this pure-fname
                   ;; bcs :TREE expects bigger indent if we "enter" into path deeper:
                   (not (subpath-p last-state-pure-fname fname :equal-too nil)))
              (if state-fnames
        ...
        (when parsed-line
          ...
          (incf (<parsed-lines>-tree-hits PARSED-LINES))
          (setf last-state-pure-fname pure-fname)
          parsed-line)))))
...
(defun parse-line-as-ls1r (PARSED-LINES STR)
  ...
  (let* ((last-i (1- (length STR)))
         ...
         (fname-extraction (extract-ls-fname (if str-is-dir-hint
                                                 (subseq STR 0 last-i)
                                                 STR)))
         (fmode (when (not str-is-dir-hint) (parse-fmode-str STR *fmode-bits*)))
         (fname (when (and (eq 'fname (car fname-extraction))
                           ;; TODO good, but leads to 2 fails: it recognize fname=1 when STR=total 1.
                           ;; TODO fname=a b is also shorter then STR='a b', so criteria must be smarter.
                           ;; TODO Also ls1rl must count 'total X' as its line, but it cannot add it to parsed lines
                           ;; TODO so maybe to add some counter to <parsed-lines> - not only lines number must be taken
                           ;; TODO into account?
                           ;; (= (length STR) (length (cdr fname-extraction)))
                           )
                  (cdr fname-extraction)))
         (fail-msg (when (eq 'fail (car fname-extraction)) (cdr fname-extraction))))
    (when (and (>= last-i 0) (not fmode))
      ;; (dbg (format nil "~%!!!!! LS1R got it !!!!! STR: ~A fmode: ~A~%" STR fmode))
      (symbol-macrolet
        ...
        (cond
          ((and str-is-dir-hint fname)  ;; Hint of yet another directory ("xyz:")
           ...
           (incf (<parsed-lines>-ls1r-hits PARSED-LINES))
...
(defun parse-line-as-ls1rl (PARSED-LINES STR)
  ...
  (let* ((last-i (1- (length STR)))
         ...
         (fail-msg (when (eq 'fail (car fname-extraction)) (cdr fname-extraction))))
    (cond
      ((and str-is-not-empty (not str-is-total-hint))
       ;; (dbg (format nil "~%!!!!! LS1RL got it !!!!! STR: ~A fmode: ~A~%" STR fmode))
       (symbol-macrolet
            ...
            (multiple-value-bind (fname-pos fname-exists) (gethash fname state-fname-hashes)
              (if fname-exists
                  ...
                  (setf (gethash fname state-fname-hashes)
                        (hash-table-count state-fname-hashes))))
            (setf state-dir fname)
            (incf (<parsed-lines>-ls1rl-hits PARSED-LINES))
...
returns a list of lists (parsed strings) or null if nothing can be parsed more"
  ;; (format t "STR: ~A~%AF: ~A PF: ~A AFPL: ~A~%"
  ;;         STR
  ;;         (get-most-parsed-lines PARSED-LINES)
  ;;         (mapcar #'get-parse-func (mapcar #'car (get-most-parsed-lines PARSED-LINES)))
  ;;         (progn
  ;;            (dolist (fmt +formats+) (reset-parsed-lines-fails PARSED-LINES fmt))
  ;;                    (parse-line-as-tree PARSED-LINES STR)
  ;;                    )
  ;;         )
           ...
           (progn
             ...
             (remove nil
                     (mapcar (lambda (parse-func) (funcall parse-func PARSED-LINES STR))
                             parse-funcs)))))
    ;; (dbg (format nil "~%!!!!!!!!!!!!!!! STR:'~A'~%  active-formats: ~A~%" STR active-formats))
    (when (and (not active-format-parsed-lines) (<parsed-lines>-verbose PARSED-LINES))
      ...
      (dolist (fmt +formats+)
        (let ((fmt-fail-msg (get-parsed-lines-fail-msg PARSED-LINES fmt)))
          (when fmt-fail-msg (format *error-output* "*** ERROR [~A]: ~A~%" fmt fmt-fail-msg))))) ;; TODO more errors
...
(defun replace-all-occurrences (IN-STR OLD NEW)
  (let* ((in-len (length IN-STR))
         (old-len (length OLD))
         (jump-over-old (max 1 old-len))
         done
         res-str)
    (labels ((repl (from out-str)
               (let* ((old-pos (if (> from in-len)
                                   nil
                                   (search OLD IN-STR :start2 from)))
                      (just-prev (and (= 0 old-len) (> from 0) (<= from in-len))))
                 (cond
                   (old-pos
                    (setf done t)
                    (if just-prev
                        (write-string IN-STR out-str :start (1- from) :end from)
                        (write-string IN-STR out-str :start from :end old-pos))
                    (write-string NEW out-str)
                    (repl (+ old-pos jump-over-old) out-str))
                   ((< from in-len)
                    (write-string IN-STR out-str :start from))))))
      (setf res-str (with-output-to-string (res)
                      (repl 0 res)))
      (values res-str done))))
...
(defmacro with-fname (PARSED-LINE
  ...
  (let ((on-dir nil)
    ...
    `(let* ((,g-quote-path
              (lambda (f)
                (format nil "'~A'" (replace-all-occurrences f "'" "'\\''"))))
...
(5am:test markup-line-as-tree--test5
          (5am:is (equal '((fname . ".") (fname-start . 0) (fname-stop))
                         (markup-line-as-tree "."))))
...
(5am:test parse-line-as-tree--test6
          (5am:is (equalp
                   (make-<parsed-lines>
                    :dir-from-final-slash t
                    :tree-hits 3
                   ...
                   (let ((pls (make-<parsed-lines>)))
                     ...
                     (parse-line-as-tree pls "└── opened.txt")
                     pls))))
...
(5am:test split-path--test1
          (5am:is (equal
                   '("" "a" "b" "")
                   (split-path "/a/b/"))))

(5am:test split-path--test2
          (5am:is (equal
                   '("" "a" "b")
                   (split-path "/a/b/" :strip-end t))))

(5am:test split-path--test3
          (5am:is (equal
                   '("")
                   (split-path ""))))

(5am:test split-path--test4
          (5am:is (equal
                   '("") ;; TODO mb nil ?
                   (split-path "" :strip-end t))))

(5am:test subpath-p--test1
          (5am:is (equal
                   nil
                   (subpath-p nil "a/b"))))

(5am:test subpath-p--test2
          (5am:is (equal
                   nil
                   (subpath-p "a/b" nil))))

(5am:test subpath-p--test3
          (5am:is (equal
                   t
                   (subpath-p "a/b" "a/b/c"))))

(5am:test subpath-p--test4
          (5am:is (equal
                   nil
                   (subpath-p "a" "abc"))))

(5am:test subpath-p--test5
          (5am:is (equal
                   nil
                   (subpath-p "a" "/a/b"))))

(5am:test subpath-p--test6
          (5am:is (equal
                   nil
                   (subpath-p "/a" "a/b"))))

(5am:test subpath-p--test7
          (5am:is (equal
                   t
                   (subpath-p "a/b/" "a/b/c"))))

(5am:test subpath-p--test8
          (5am:is (equal
                   t
                   (subpath-p "a/b/c" "a/b/c"))))

(5am:test replace-all-occurrences--test1
          (5am:is (equal
                   '("XbcX" t)
                   (multiple-value-list (replace-all-occurrences "abca" "a" "X")))))

(5am:test replace-all-occurrences--test2
          (5am:is (equal
                   '("abc" NIL)
                   (multiple-value-list (replace-all-occurrences "abc" "X" "Y")))))

(5am:test replace-all-occurrences--test3
          (5am:is (equal
                   '("abc" NIL)
                   (multiple-value-list (replace-all-occurrences "abc" "X" "Y")))))

(5am:test replace-all-occurrences--test4
          (5am:is (equal
                   '("" NIL)
                   (multiple-value-list (replace-all-occurrences "" "X" "Y")))))

(5am:test replace-all-occurrences--test5
          (5am:is (equal
                   '("Y" t)
                   (multiple-value-list (replace-all-occurrences "" "" "Y")))))

(5am:test replace-all-occurrences--test6
          (5am:is (equal
                   '("YaYbYcY" t)
                   (multiple-value-list (replace-all-occurrences "abc" "" "Y")))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; Command line ;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun cli-kw-getopt (cmd key)
  "Allows to get a choice/enum option as a symbol in :keyword package ready for eq/getf/..."
  (let ((opt (clingon:getopt cmd key)))
    (when opt
