...
(5am:test extract-fname-and-indent--empty
  (5am:is (equal nil
                 (acl2::extract-fname-and-indent ""))))

(5am:test extract-fname-and-indent--dot
  (5am:is (equal nil
                 (acl2::extract-fname-and-indent "."))))

(5am:test extract-fname-and-indent--in-tree
  (5am:is (equal '("file A" . 6)
                 (acl2::extract-fname-and-indent "  +-- file A"))))

(5am:test extract-fname-and-indent--in-trees
  (5am:is (equal '("file A" . 7)
                 (acl2::extract-fname-and-indent " | +-- file A"))))

(5am:test extract-fname-and-indent--with-star
  (5am:is (equal '("\"build.1.sh\"*" . 4)
                 (acl2::extract-fname-and-indent "├── \"build.1.sh\"*"))))

(5am:test join-path-components
  (5am:is (equal "a1/b2/c3"
                 (acl2::join-path-components '("a1" "b2" "c3")))))
...
(5am:test
 adapt-qf-fname--1
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::adapt-qf-fname "file/")))
   (5am:is (equal (cons "file" :directory) res))))

(5am:test
 adapt-qf-fname--2
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::adapt-qf-fname "file*")))
   (5am:is (equal (cons "file" :executable) res))))
...
(5am:test
 adapt-qf-fname--4
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::adapt-qf-fname "*")))
   (5am:is (equal (cons "*" :file) res))))


;; TODO more such tests:
(5am:test
    parse-tree-output--line-with-indent-1
 (5am:is
  (equal
   '("/etc")
   (acl2::parse-tree-output '("/etc")))))

(5am:test
 parse-tree-output--line-with-indent-2
  (let* ((acl2::*dbg-cond* nil)
         (res (acl2::parse-tree-output
               '("."
                 "├── package.lisp"
                 "├── P L/"
                 "│   ├── A/"
                 ...
                 "│   │   └── x"))))
    (5am:is (equal
             '("." "./package.lisp" "./P L/" "./P L/A/" "./P L/ascetic.min.css" "./P L/some.js" "./P L/B/" "./P L/B/x")
             res))))

(5am:test
 parse-tree-output--line-with-indent-3
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::parse-tree-output
              '("\".\""
                "+-- \"package.lisp\""
...
(5am:test
 parse-tree-output--line-with-indent-4
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::parse-tree-output
              '("\"/etc\"/"
                "+-- \"package.lisp\""
   ...
   (5am:is (equal
            '("/etc/"
              ...
              "/etc/P L/"
              "/etc/P L/A/"
              "/etc/P L/ascetic.min.css"
              "/etc/P L/some.js"
              "/etc/P L/B/")
            res))))

(5am:test
 parse-tree-output--line-with-indent-5
 (let* ((acl2::*dbg-cond* nil)
        (res (acl2::parse-tree-output
              '("\".\"/"
              "├── \"*\""
              ...
              "├── \"acl2-utils.lisp_\""
              "├── \"acl2-utils.lisp~\""
              "├── \"ASDF-vs-QUICKLISP.md\""
              "├── \"build.1.sh\"*"
              "├── \"build.log\""
              "├── \"build.sh\"*"
              "├── \"build.sh~\"*"
              "├── \"DetectFormat1.drakon\""))))
   (5am:is (equal  ;; pay attention: without not ensurable file objects (ends with * and similar)
            '("./"
              "./*"
              "./$"
              "./ACL2-AXIOMS.md"
...
(5am:test
 classify-tree-output--1
  (let* ((acl2::*dbg-cond* nil)
         (res0 (acl2::parse-tree-output
               '("\".\""
                 "+-- \"package.lisp\""
                 "+-- \"P L\"/"
                 "|   +-- \"A\"/"
                 ...
                 "|   +-- \"B\"/"
                 "|   |   +-- \"   x\"")))
         (res1 (acl2::classify-tree-output res0)))
    (5am:is (equal
             '((:special ".")
               (:file "./package.lisp")
               (:directory "./P L/")
               (:directory "./P L/A/")
               (:file "./P L/ascetic.min.css")
               (:file "./P L/some.js")
               (:directory "./P L/B/")
               (:file "./P L/B/   x"))
             res1))))
...
(5am:test
    prepend-classified-tree-output--1
  (let* ((acl2::*dbg-cond* nil)
         (res0 (acl2::parse-tree-output
                '("\".\""
                  "+-- \"package.lisp\""
                  ...
                  "|   |   +-- \"   x\"")))
         (res1 (acl2::classify-tree-output res0))
         (res2 (acl2::prepend-classified-tree-output "a/b/c/" res1)))
    (5am:is (equal
             '((:directory "a/b/c/")
               (:special ".")
               (:file "a/b/c/package.lisp")
               (:directory "a/b/c/P L/")
               (:directory "a/b/c/P L/A/")
               (:file "a/b/c/P L/ascetic.min.css")
               (:file "a/b/c/P L/some.js")
               (:directory "a/b/c/P L/B/")
               (:file "a/b/c/P L/B/   x"))
             res2))))

(5am:test
    ensure-classified-tree-output--1
  (let* ((acl2::*dbg-cond* nil)
         (res0 (acl2::parse-tree-output
                '("\".\""
                  "+-- \"package.lisp\""
         ...
         (res1 (acl2::classify-tree-output res0))
         (res2 (acl2::prepend-classified-tree-output "a/b/c/" res1))
         (res3 (with-output-to-string (*standard-output*)
                 (easytree::ensure-classified-tree-output res2 :script nil nil))))
    (5am:is (equal
             "[ ! -e 'a/b/c/' ] && { mkdir -p 'a/b/c/'; }
[ ! -e 'a/b/c/package.lisp' ] && { touch 'a/b/c/package.lisp'; }
[ ! -e 'a/b/c/P L/' ] && { mkdir -p 'a/b/c/P L/'; }
[ ! -e 'a/b/c/P L/A/' ] && { mkdir -p 'a/b/c/P L/A/'; }
[ ! -e 'a/b/c/P L/ascetic.min.css' ] && { touch 'a/b/c/P L/ascetic.min.css'; }
[ ! -e 'a/b/c/P L/some.js' ] && { touch 'a/b/c/P L/some.js'; }
[ ! -e 'a/b/c/P L/B/' ] && { mkdir -p 'a/b/c/P L/B/'; }
[ ! -e 'a/b/c/P L/B/   x' ] && { touch 'a/b/c/P L/B/   x'; }
"
             res3))))

(5am:test
    ensure-classified-tree-output--2
  (let* ((acl2::*dbg-cond* nil)
         (res0 (acl2::parse-tree-output
                '("\".\""
                  "+-- \"package.lisp\""
         ...
         (res2 (acl2::prepend-classified-tree-output "a/b/c/" res1))
         (res3 (with-output-to-string (*standard-output*)
                 (easytree::ensure-classified-tree-output res2 :script t nil))))
    (5am:is (equal
"[ ! -e 'a/b/c/' ] && echo '[DRY] Creating directory a/b/c/ ...'
[ ! -e 'a/b/c/package.lisp' ] && echo '[DRY] Creating file a/b/c/package.lisp ...'
[ ! -e 'a/b/c/P L/' ] && echo '[DRY] Creating directory a/b/c/P L/ ...'
[ ! -e 'a/b/c/P L/A/' ] && echo '[DRY] Creating directory a/b/c/P L/A/ ...'
[ ! -e 'a/b/c/P L/ascetic.min.css' ] && echo '[DRY] Creating file a/b/c/P L/ascetic.min.css ...'
[ ! -e 'a/b/c/P L/some.js' ] && echo '[DRY] Creating file a/b/c/P L/some.js ...'
[ ! -e 'a/b/c/P L/B/' ] && echo '[DRY] Creating directory a/b/c/P L/B/ ...'
[ ! -e 'a/b/c/P L/B/   x' ] && echo '[DRY] Creating file a/b/c/P L/B/   x ...'
"
             res3))))
...
(defmacro it-exists (var gen-spec &body body)
  "Existense test (for generated values, kind of property tests)"
  (let ((g (gensym "GEN"))
        (val (gensym "VAL")))
    `(let ((,g ,gen-spec))
       (ignore-errors
